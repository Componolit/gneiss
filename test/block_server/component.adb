...
package body Component is
   ...
   type Disk is array (Block.Id range 0 .. 131071) of Block_Buffer;
   ...
   begin
      R.Status := Block.Error;
      --  if
      --     Buf'Length mod Block_Buffer'Length = 0 and then
      --     R.Start in Ram_Disk'Range and then
      --     R.Start + (R.Length - 1) in Ram_Disk'Range
      --  then
      Block_Server.Write (Server, R, Buf);
      for I in Block.Id range R.Start .. R.Start + (R.Length - 1) loop
         Ram_Disk (I) :=
            Buf (Buf'First + (I - R.Start) * Block_Buffer'Length ..
               Buf'First + ((I - R.Start) + 1) * Block_Buffer'Length - 1);
      end loop;
      R.Status := Block.Ok;
      --  end if;
   end Write;
   ...
   begin
      ...
      Componolit.Interfaces.Log.Client.Info (Log, "Event");
      if Block_Server.Initialized (Server) then
         loop
            R := Block_Server.Head (Server);
            case R.Kind is
               when Block.Read =>
                  Componolit.Interfaces.Log.Client.Info (Log, "Read");
                  Read (R);
                  while R.Status /= Block.Acknowledged loop
                     Block_Server.Acknowledge (Server, R);
                  end loop;
               when Block.Write =>
                  Componolit.Interfaces.Log.Client.Info (Log, "Write");
                  loop
                     null;
                  end loop;
                  Write (R);
               ...
               when others =>
                  Componolit.Interfaces.Log.Client.Info (Log, "Other");
                  --  loop
                  --     null;
                  --  end loop;
            end case;
      ...
      Componolit.Interfaces.Log.Client.Info (Log, "Event return");
   ...
   is
      pragma Unreferenced (S);
   begin
      Componolit.Interfaces.Log.Client.Info (Log, "Finalized");
   end Finalize;
